...
impl Meshable for Sphere {
    fn get_vecs(
        ...
        let delta_latitude = std::f32::consts::PI / self.resolution as f32;
        let delta_longitude = 2.0 * std::f32::consts::PI / self.resolution as f32;

        // TODO: Correct sizes?
        let mut positions = Vec::with_capacity(self.resolution as usize * self.resolution as usize);
        let mut normals = Vec::with_capacity(self.resolution as usize * self.resolution as usize);
        let mut uvs = Vec::with_capacity(self.resolution as usize * self.resolution as usize);
        // FIXME: Definitely not the correct size
        let mut indices = Vec::with_capacity(self.resolution as usize * self.resolution as usize);
        ...
        */
        for i in 0..=self.resolution {
            ...
            let mut index_1 = i as u32 * (self.resolution as u32 + 1);
            let mut index_2 = index_1 + self.resolution as u32 + 1;
            ...
            for j in 0..=self.resolution {
                ...
                let uv = [j as f32 / self.resolution as f32, i as f32 / self.resolution as f32];
